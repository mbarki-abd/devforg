...
async function main() {
  ...
  // Try to delete existing executions collection if exists
  console.log('Attempting to delete existing executions collection...');
  const deleteResponse = await request('POST', '/api/collections:destroy?filterByTk=executions', null, token);
  console.log('Delete response:', JSON.stringify(deleteResponse));

  // Recreate executions collection with proper schema
  console.log('\nCreating executions collection...');
